...
public class Fitness {
    ...
    public void processSubscription(Subscription sub, FitnessZone zone) {
        ...
        if (!isZoneAllowed || !isTimeAllowed) {
            System.out.println("Доступ запрещен: Проверьте зону и время.");
            return;
        }

        Subscription[] targetArray = (zone == FitnessZone.SWIMPOOL) ? poolMembers : (zone == FitnessZone.GROUP_EXERCISE) ? groupMembers : gymMembers;

        for (int i = 0; i < 20; i++) {
            if (targetArray[i] != null && targetArray[i].getOwner().getId() == sub.getOwner().getId()) {
                System.out.println("Вы уже внутри.");
                return;
            }
        }
        ...
        for (int i = 0; i < 20; i++) {
            if (targetArray[i] == null) {
                ...
                db.recordAttendance(sub.getOwner().getId(), zone.toString());
                return;
            }
        }
